...
export default function ModelSelector({ models, selectedModel, onSelect, disabled }: ModelSelectorProps) {
	...
	return (
		<div>
			<label className='block text-sm font-medium text-stone-600 mb-3'>Select Model</label>
					...
					return (
						...
						>
							...
							>
								{Icon && (
									<Icon
										className={clsx("w-5 h-5", isSelected ? "text-teal-600" : "text-stone-500")}
									/>
								)}
							</div>

							<span
								className={clsx("text-sm font-medium", isSelected ? "text-teal-700" : "text-stone-600")}
							>
								{model.name}
							</span>
						</motion.button>
